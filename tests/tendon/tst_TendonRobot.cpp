...
TEST(TendonRobotTests, home_shape_same_as_zero_shape) {
  ...
  for (size_t i = 0; i < shape.p.size(); i++) {
    ASSERT_NEAR((shape.p[i] - home_shape.p[i]).norm(), 0.0, 1e-10)
      << "  p[" << i << "] = " << shape.p[i]
      << ", home.p[" << i << "] = " << home_shape.p[i];
    ASSERT_NEAR(shape.t[i], home_shape.t[i], 1e-10) << "  for i = " << i;
  }
...
TEST(TendonRobotTests, shape_dL_larger_than_L) {
  ...
  ASSERT_EQ(shape.t[0], 0);
  ASSERT_EQ(shape.t[1], 1);
}
...
TEST(TendonRobotTests, shape_dL_larger_than_L_minus_s_start) {
  ...
  auto shape = robot.shape(state);
  ASSERT_EQ(shape.t.size(), size_t(2));
  ASSERT_EQ(shape.t[0], s_start);
  ASSERT_EQ(shape.t[1], robot.specs.L);
}
...
TEST(TendonRobotTests, shape_s_start_negative) {
  auto robot = create_robot();
  robot.specs.L = 0.95;
  robot.specs.dL = 0.01;
  double s_start = -0.01;
...
TEST(TendonRobotTests, shape_s_start_greater_than_L) {
  auto robot = create_robot();
  robot.specs.L = 0.95;
  robot.specs.dL = 0.01;
  double s_start = 1.00;
...
TEST(TendonRobotTests, home_shape_dL_larger_than_L) {
  ...
  ASSERT_EQ(home_shape.t[0], 0);
  ASSERT_EQ(home_shape.t[1], 1);
}
...
TEST(TendonRobotTests, home_shape_dL_larger_than_L_minus_s_start) {
  ...
  ASSERT_EQ(home_shape.t.size(), size_t(2));
  ASSERT_EQ(home_shape.t[0], s_start);
  ASSERT_EQ(home_shape.t[1], robot.specs.L);
}
...
TEST(TendonRobotTests, home_shape_s_start_greater_than_L) {
  auto robot = create_robot();
  robot.specs.L = 0.95;
  robot.specs.dL = 0.01;
  double s_start = 1.00;
...
TEST(TendonRobotTests, home_shape_s_start_negative) {
  auto robot = create_robot();
  robot.specs.L = 0.95;
  robot.specs.dL = 0.01;
  double s_start = -0.01;
