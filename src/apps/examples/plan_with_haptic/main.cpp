...
namespace defaults {
  ...
  const std::string  ompl_log_level  = "WARN";
...
void populate_parser(CliParser &parser) {
  ...
  parser.add_flag("--skip-roadmap-check");
  parser.set_description("--skip-roadmap-check",
                      "By default when loading a roadmap, the vertices and\n"
      "                edges are checked for validity at the time of\n"
      "                loading.  This option disables that validity check at\n"
      "                loading time and instead does lazy evalutation during\n"
      "                planning.");
...
int main(int arg_count, char* arg_list[]) {
  ...
  auto check_validity  = !parser.has("--skip-roadmap-check");
  ...
  env_publisher.set_color(0.0f, 0.0f, 1.0f, 0.4f); // transparent blue
